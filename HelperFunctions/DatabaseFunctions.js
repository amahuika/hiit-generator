...
export function createTable() {
  ...
  db.transaction((tx) => {
    ...
    tx.executeSql(
      "CREATE TABLE IF NOT EXISTS rounds (id INTEGER PRIMARY KEY AUTOINCREMENT, name VARCHAR(255))",
      []
    );
    tx.executeSql(
    ...
    tx.executeSql(
      "CREATE TABLE IF NOT EXISTS workout_junction (id INTEGER PRIMARY KEY AUTOINCREMENT, workout_id INTEGER REFERENCES saved_workout(id), exercise_id INTEGER REFERENCES exercises(id), round_id INTEGER REFERENCES rounds(id))",
      []
    );
  });
}
